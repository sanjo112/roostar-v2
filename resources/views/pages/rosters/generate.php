<section class="generation-page">
  ...
  <div class="generation-dashboard has-context-side">
    ...
    <aside class="generation-main-column">
      ...
      <?php if (!empty($engineResult)): ?>
        <section class="card generation-status-card">
          <div class="generation-panel-head">
            <div>
              <div class="eyebrow">Engine V2</div>
              <h2>Modulaire pipeline</h2>
            </div>
            <span class="status <?= $engineResult->score->validation->isValid() ? 'st-done' : 'st-block' ?>">
              <?= $engineResult->score->validation->isValid() ? 'Geldig' : 'Ongeldig' ?>
            </span>
          </div>

          <div class="generation-steps">
            <?php foreach (($engineResult->steps ?? []) as $step): ?>
              <div class="generation-step <?= !empty($step['valid']) ? 'done' : 'error' ?>">
                <div class="generation-step-icon"><span><?= !empty($step['valid']) ? '✓' : '!' ?></span></div>
                <div>
                  <strong><?= htmlspecialchars((string) $step['name']) ?></strong>
                  <span>Score <?= htmlspecialchars((string) $step['score']) ?> · hard <?= htmlspecialchars((string) $step['hard_count']) ?> · zacht <?= htmlspecialchars((string) $step['soft_count']) ?></span>
                </div>
              </div>
            <?php endforeach; ?>
          </div>
          ...
          <div class="editor-issue-list engine-explanation-list">
            <?php foreach (array_slice($engineResult->explanations ?? [], 0, 4) as $explanation): ?>
              <div class="editor-issue <?= $explanation['severity'] === 'hard' ? 'hard' : 'soft' ?>">
                ...
                <span><?= htmlspecialchars((string) $explanation['message']) ?></span>
              </div>
            <?php endforeach; ?>
            <?php if (empty($engineResult->explanations)): ?>
              <div class="editor-issue soft"><strong>Geen blokkades</strong><span>De modulaire demo-engine vond geen harde fouten.</span></div>
            <?php endif; ?>
          </div>
        </section>
      <?php endif; ?>
